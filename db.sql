...
CREATE TABLE partida_guardada_tbl (
  ...
  FOREIGN KEY (id_jugador) REFERENCES jugadores_tbl(id) ON DELETE CASCADE ON UPDATE CASCADE,
  UNIQUE (id_jugador) 
);
...
CREATE TABLE acciones_usuario_tbl (
  ...
  FOREIGN KEY (usuario_id) REFERENCES jugadores_tbl(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
INSERT INTO jugadores_tbl (nombre_completo, telefono, fecha_nacimiento, email, contrasena, foto_perfil, nivel, estatus) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
...
SELECT * FROM jugadores_tbl WHERE email = ? OR telefono = ?;

-- 9. Obtener usuario por email
SELECT * FROM jugadores_tbl WHERE email = ?;

-- 10. Actualizar la última sesión del usuario
UPDATE jugadores_tbl SET ultima_sesion = CURRENT_TIMESTAMP WHERE email = ?;

-- 11. Buscar usuario por ID, email o teléfono
SELECT * FROM jugadores_tbl WHERE id = ? OR email = ? OR telefono = ?;
...
UPDATE jugadores_tbl SET estatus = CASE WHEN estatus = 'ACTIVO' THEN 'INACTIVO' WHEN estatus = 'INACTIVO' THEN 'ACTIVO' END WHERE id = ?;

-- 20. Modificar el nivel de un usuario
UPDATE jugadores_tbl SET nivel = CASE WHEN nivel = 'ADMIN' THEN 'USUARIO' WHEN nivel = 'USUARIO' THEN 'ADMIN' ELSE nivel END WHERE id = ?;